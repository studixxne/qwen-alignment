...
def run_dpo_llm_judge(config: JudgeConfig):
    ...
    for step, item in enumerate(dataset):
        ...
        judge, comment = response['judge'], response['comment']
        item['judge'], item['comment'] = judge, comment
        results.append(item)
        ...
        if judge == "A":
            ...
            status_str = f"{Color.RED}{Color.BOLD}INVALID / API ERROR{Color.RESET}"
    ...
    if counts['INVALID'] > 0:
        tqdm.write(f"{Color.RED}API ERROR{Color.RESET}       : {counts['INVALID']}개 ({counts['INVALID']/total*100:.1f}%)")
